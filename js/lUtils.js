...
/**
 * 发送错误信息
 * @param {Object} type	信息类型error或其他
 * @param {Object} id	项目信息
 * @param {Object} info	信息json字符串
 */
function lSendError(type,id,info,onCom) {
	
	
	if(lTyepof(info)=="Object")
	{
		try{
			info = JSON.stringify(info)
		}catch(e){
			console.log("lSendError",e);
		}
	}
	
	lAjax("http://logcollection.sinaapp.com/sendinfo.php",{type:type,id:id,info:info},onSu)
	
	function onSu (d) {
		console.log(d);
		console.log("提交错误");
		console.log("类型:"+type,"项目:"+id,"发送信息：",info);
		
		if(onCom!=undefined) onCom(d)
	}
...
window.onerror = function(errorMessage, scriptURI, lineNumber,columnNumber,errorObj) 
{
	...
	{
		var infoObj={}
		infoObj.errorMessage = errorMessage
		infoObj.scriptURI = scriptURI
		infoObj.lineNumber = lineNumber
		infoObj.columnNumber = columnNumber
		infoObj.errorObj = errorObj
		
		var pName = "windowOnerror"
		...
		lSendError("error",pName,infoObj)
	}*/
